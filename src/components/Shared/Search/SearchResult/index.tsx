...
export default function SearchResult({ hits, setHasFocus, closebothNavBar, searchTitle }: ISearchResultProps) {
    ...
    function highlight(hit: any) {
        if (hit._highlightResult?.title?.matchedWords?.length > 0) {
            return (
                <span
                    className={`noWrap ${styles.hitWeighTitle}`}
            ...
            return (
                <span className={`${styles.hitContent} noWrap`}>
                    ...
                    <small
                        className="noWrap"
                        dangerouslySetInnerHTML={{ __html: handleContentHit(hit._highlightResult?.internal?.content?.value) }}
                    />
                </span>
            );
        }
    ...
    return (
        <>
            <p className={`${styles.results_portal_title} halfTitleEyebrow`} onClick={() => hiddenResults()}>
                {searchTitle} <span className={`${hideResults && styles.rotateSvg}`}>{icons.chevronDown}</span>
            </p>
                                    ...
                                    <div className={`${styles.results_container_hit}`} key={`${hit.objectID}-${i}`}>
                                        <div className={styles.results_container_hit_link}>
                                            ...
                                            <div className={` ${styles.docElement}`}>
                                                {highlightDoc(hit)?.map((parsedHit: any) => parsedHit)}
                                            </div>
                                        </div>
